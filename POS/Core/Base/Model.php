...
class Model
{
    ...
    {
        $data = array();
        $result = $this->connection->query("SELECT * FROM products ORDER BY cost_price DESC LIMIT 5
        ");
    ...
    {
        $stmt = $this->connection->prepare("SELECT * FROM transactions WHERE user_id=?"); // prepare the sql statement
        $stmt->bind_param('i', $user_id); // bind the params per data type (https://www.php.net/manual/en/mysqli-stmt.bind-param.php)
